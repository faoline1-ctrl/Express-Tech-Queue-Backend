...
// Start work order: create or increment technician
app.post('/startWorkOrder', (req, res) => {
  ...
  app.post('/setStatus', (req, res) => {
    ...
    tech.status = status;
    if (status === 'Available') tech.status_timestamp = new Date().toISOString();
    try { writeQueue(queue); } catch (err) { console.error(err); return res.status(500).json({ error: 'Persist failed' }); }
    broadcastQueue(queue);
  ...
  function broadcastQueue(queue) {
    ...
    for (const r of Array.from(sseClients)) {
      try { r.write(payload); } catch (e) { sseClients.delete(r); try { r.end(); } catch (e) {} }
    }
  }
